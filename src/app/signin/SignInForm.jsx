    ...
    <form className="w-full mt-5" onSubmit={handleSubmit}>
      <div className="mb-4">
        ...
        <input
          ...
          required
        />
        <p className={`min-h-[1.1rem] text-red-400 text-xs mt-1 ${emailCheck ? "block" : "hidden"}`}>User does not exist</p>
      </div>

      <div className="mb-2">
        <label htmlFor="password" className="block mb-1.5 text-sm font-medium text-paper">Your password</label>
        <div className="relative">
          <input
            type={showPassword ? 'text' : 'password'}
            id="password"
            className="text-sm rounded-lg block w-full p-2.5 pr-11 bg-ink border border-white/10 placeholder-mist-dim text-paper focus:outline-none focus:ring-2 focus:ring-gold/50 focus:border-gold/50 transition-shadow"
          ...
          <button
            ...
            className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-mist-dim hover:text-gold transition-colors cursor-pointer"
          >
            {showPassword ?
        ...
        <p className={`min-h-[1.1rem] text-red-400 text-xs mt-1 ${passwordCheck ? "block" : "hidden"}`}>Incorrect password</p>
      </div>
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center">
          ...
          />
          <label htmlFor="remember" className="ms-2 text-sm text-mist cursor-pointer">Remember me</label>
        </div>
      </div>

      <button
        type="submit"
        className="text-ink bg-gold hover:bg-gold-soft focus:outline-none font-semibold rounded-lg text-sm w-full py-2.5 text-center cursor-pointer transition-colors duration-300"
      >
        Sign in
      </button>
    </form>
